...
interface PolymorphicHydrationType {
    key: string;
...
export abstract class AbstractPolymorphicRepository<E> extends Repository<E> {
    private getPolymorphicMetadata(): PolymorphicMetadataInterface[] {
        ...
        if (Array.isArray(entities)) {
            return Promise.all(
                entities.map((polymorph) => {
                    polymorph[entityIdColumn(options)] =
                ...
                (options: PolymorphicMetadataInterface) =>
                    new Promise(async (resolve) =>
                        options.cascade
                            ? resolve({
                                  key: options.propertyKey,
                                  entities: await this.savePolymorhic(
                                      entity,
                                      options,
                                  ),
                              })
                                ...
                                entityTypes.map(
                                    (type: () => Function | Function[]) => {
                                        ...
                                        repository.delete({
                                            ...
                                            [entityIdColumn(option)]: entity[
                                                PrimaryColumn(option)
                                            ],
                                        });
                                    },
                                ),
    ...
    private findRepository(
        ...
        const repository: Repository<PolymorphicChildInterface | never> =
        ...
        metadata.map((options: PolymorphicOptionsType) => {
            if (this.isParent(options)) {
                (Array.isArray(entityOrEntities)
                    ...
                    if (
                        !parent ||
                        entity[entityIdColumn(options)] !== undefined
                    ) {
                        return entity;
                    }
                    entity[entityIdColumn(options)] =
